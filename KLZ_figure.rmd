...
### Defining admission and discharge time point 
...
env1 <- dplyr::select(map_A,Sample,Age,Gender,CORTICOSTEROID,Death,
                      COMORBIDITY,Treatment,Severity_A,Copy,ANTB_CLS3_STATUS,Antibiotic,
                      Individual_ID)
...
gamlss_t1 <- taxa.compare(taxtab =df1,propmed.rel="gamlss",comvar="Death",
                          personid = "Individual_ID",adjustvar =c("CORTICOSTEROID","Treatment","Gender","Age","Severity_A","COMORBIDITY","Antibiotic"),
...
genus_A=(genus[,metadata %>% filter(Death %in% c(1,0),reads_gthan3000=="yes") %>% .[,"Sample"]] %>% as.matrix() %>% prop.table(.,2) %>% .[rowSums(.>0.01)>0,] %>% .[,map_A$Sample]%>% prop.table(.,2))
gam_genus_A=rownames(tt2)[tt2$Death1<0.05]
gam_genus_A=gam_genus_A[order(df1[gam_genus_A,"Death1"],decreasing = T)]
##Odds ratio
row_df=data.frame(genus=gam_genus_A,OR=df1[gam_genus_D,"Death1"])  
colnames(genus_A)=gsub("k__bacteria.g__","",colnames(genus_A))
plot_df2=genus_A[gam_genus_A,]
plot_df2=log10(plot_df2+1e-5)
col_df=data.frame(samples=colnames(plot_df2),class=map_A[colnames(plot_df2),"Death"])
...
gamlss_t2 <- taxa.compare(taxtab =df1,propmed.rel="gamlss",comvar="Death",personid = "Individual_ID",
                          adjustvar =c("CORTICOSTEROID","Treatment",
                                       "Gender","Age","Severity_A","Antibiotic","COMORBIDITY","ANTB_CLS3_STATUS"),
...
```
##LefSE analysis was performed on galaxy website

## Figure S2B relative abundance heatmap
```{r,echo=TRUE,tidy=TRUE}
genus_D=(genus[,metadata %>% filter(Death %in% c(1,0),reads_gthan3000=="yes") %>% .[,"Sample"]] %>% as.matrix() %>% prop.table(.,2) %>% .[rowSums(.>0.01)>0,] %>% .[,map_D$Sample]%>% prop.table(.,2))
...
## Figure 6C
```{r,echo=TRUE,tidy=TRUE}
plasma=read_excel("pheatmap_df.xlsx",sheet = 1) %>% as.data.frame()
index=select(plasma,Sample_ID2,index)
names(index)[1]="Sample_ID"
vars=c('WBC','NEU' ,'LYM','ESR','PCT',
       'IL-1b', 'IL-1ra','IL-2', 'IL-4','IL-5', 'IL-6', 'IL-7', 'IL-8', 'IL-9', 'IL-10', 'IL-12', 'IL-13',
       'IL-15', 'IL-17', 'Eotaxin', 'IP-10', 'MCP-1(MCAF)', 'MIP-1a', 'MIP-1b',
       'RANTES', 'FGF basic', 'PDGF-bb', 'VEGF', 'G-CSF', 'GM-CSF', 'IFN-g',
       'TNF-a')
plasma=dplyr::select(plasma,Sample_ID2,vars)
names(plasma)[1]="Sample_ID"
rownames(plasma)=plasma$Sample_ID

t1=read.csv("t1.csv")
t1=t1$T1 
t2=read.csv("t2.csv")
t2=t2$T2
genus=read.csv("genus_filter_561.csv",row.names = 1)
species=read.csv("bind_species_1_001.csv",row.names = 1)
species=species[,index$index]
all(names(species)==index$index)
names(species)=index$Sample_ID

save.image("origin.RData")

meta_t1=meta[t1,]
meta_t2=meta[t2,]
overlap=intersect(t1,t2)
p1=meta$Individual_ID[meta$Sample_ID %in% setdiff(t1,overlap)]
p2=meta$Individual_ID[meta$Sample_ID %in% setdiff(t2,overlap)]
p12=intersect(p1,p2)

plasma_fc=data.frame(matrix(NA,length(p12),length(vars)))
rownames(plasma_fc)=p12
names(plasma_fc)=vars

for (i in rownames(plasma_fc)) {
  for(j in names(plasma_fc)){
    t1_sample=meta_t1$Sample_ID[meta_t1$Individual_ID==i]
    t2_sample=meta_t2$Sample_ID[meta_t2$Individual_ID==i]
    plasma_fc[i,j]=log2(plasma[t2_sample,j]/plasma[t1_sample,j])#####进行log2转换！！
  }
}

min(na.omit(plasma_fc))
max(na.omit(plasma_fc))

for (i in vars){
  num = sum(is.na(plasma_fc[,i]))
  if(num >0){
    print(i)
    print(num)
  }
...
plasma_fc$Individual_ID=rownames(plasma_fc) %>% as.numeric()
plasma_fc=left_join(plasma_fc,select(meta_t2,Individual_ID,Death,Age,Gender,`Severity(A)`,CORTICOSTEROID,COMORBIDITY,Treatment,invas_bin,ANTB_time,ANTB_CS3_time))
plasma_fc$ANTB_CS3_STATUS=ifelse(plasma_fc$ANTB_CS3_time==0,0,1)
...
plasma_fc_res_rmat=data.frame(matrix(NA,length(vars1),9))##9/10
rownames(plasma_fc_res_rmat)=vars1
names(plasma_fc_res_rmat)=names(plasma_fc)[c(34:40,42,44)]##40/41intubation
names(plasma_fc_res_rmat)[4]="`Severity(A)`"

plasma_fc_res_pmat=data.frame(matrix(NA,length(vars1),9))##9/10
rownames(plasma_fc_res_pmat)=vars1
names(plasma_fc_res_pmat)=names(plasma_fc)[c(34:40,42,44)]##40/41intubation
names(plasma_fc_res_pmat)[4]="`Severity(A)`"


for (i in 1:length(vars1)) {
  j=paste("`",paste(vars1[i],"`",sep = ""),sep = "")
  fm=as.formula(paste(j,paste(colnames(plasma_fc_res_pmat),collapse = "+"),sep = "~"))
  fit=lm(fm,data = plasma_fc) %>% summary()
  kk=fit$coefficients %>% as.data.frame()
  plasma_fc_res_rmat[i,]=as.numeric(kk$Estimate)[-1]
  plasma_fc_res_pmat[i,]=as.numeric(kk$`Pr(>|t|)`)[-1]
}

p.adjust.foo= function(x) p.adjust(x,method = "fdr")
plasma_fc_res_pmat = apply(plasma_fc_res_pmat,2,p.adjust.foo)

pval.df=plasma_fc_res_pmat
r.df=plasma_fc_res_rmat
sig.thres = 0.05
...
max(r.df.sig)
min(r.df.sig)
bk <- c(seq(-2,0,by=0.1),seq(0.1,2,by=0.1)) 

library(pheatmap)
colnames(r.df.sig)=c("Mortality","Corticosteroid","Treatment","High-grade antibiotics")
colnames(pval.df.sig)=c("Mortality","Corticosteroid","Treatment","High-grade antibiotics")
pheatmap(r.df.sig,
                color = colorRampPalette(c("#13457E", "white", "#A71E31"))(length(bk)),
                angle_col = "45",border=F,
                display_numbers = matrix(ifelse((pval.df.sig<=sig.thres & pval.df.sig>0.01),"*", 
                                                ifelse((pval.df.sig<=0.01 & pval.df.sig>0.001),"**",
                                                       ifelse(pval.df.sig<=0.001,"***",""))), nrow(pval.df.sig)),
                cluster_rows = F,
                cluster_columns = F,
                fontsize = 10,
                cellwidth = 15, cellheight =10,
